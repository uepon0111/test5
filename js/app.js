  ...
  async function saveArtist() {
    ...
    try {
      if (data.id) {
        ...
        if (data.iconData) {
          const artist = await Storage.getArtist(data.id);
          if (artist.iconKey) await Storage.deleteBlob(artist.iconKey);
          changes.iconKey = await Storage.saveBlob(data.iconData);
        }
        await Storage.updateArtist(data.id, changes);
        UI.toast('アーティストを更新しました', 'success');
      } else {
        let iconKey = null;
        if (data.iconData) iconKey = await Storage.saveBlob(data.iconData);
        await Storage.createArtist(data.name, iconKey);
