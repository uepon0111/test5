...
const App = (() => {
  ...
  async function saveTrackEdit() {
    ...
    try {
      ...
      for (const name of data.artistNames || []) {
        ...
        if (!trimmed) continue;
        let artist = (await Storage.getArtists()).find(a => a.name === trimmed);
        if (!artist) artist = await Storage.createArtist(trimmed);
        artists.push(artist);
      }
      const changes = {
        title:       data.title,
  ...
  async function saveArtist() {
    ...
    try {
      if (data.id) {
        await Storage.updateArtist(data.id, { name:data.name, icon:data.icon, color:data.color, textColor:data.textColor });
        UI.toast('アーティストを更新しました', 'success');
      } else {
        await Storage.createArtist(data.name, data.icon, data.color, data.textColor);
        UI.toast(`アーティスト「${data.name}」を作成しました`, 'success');
      }
      await UI.refreshArtists();
      await UI.renderArtistManager();
      UI.closeModal('artist-modal');
      Drive.triggerAutoSync();
    } catch (err) {
      console.error('Save artist error:', err);
      UI.toast('アーティストの保存に失敗しました', 'error');
    }
  }
