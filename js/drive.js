...
const Drive = (() => {
  ...
  function init() {
    ...
    // ページ終了時にペンディングのログをベストエフォートでプッシュ
  ...
  function scheduleSyncLogs() {
    clearTimeout(_logTimer);
    // 8秒後にインデックスをプッシュ（ページを閉じる前にログを保存しやすくする）
    _logTimer = setTimeout(() => { if (_token) _pushIndex().catch(() => {}); }, 8000);
  }
  ...
  async function syncNow() {
    ...
    try {
      ...
      /* Phase A-0 — ローカルの未送信ログを先にプッシュ（他端末との共有を確実に） */
      _prog(8, 'ローカルデータを保存中...');
      try {
        clearTimeout(_logTimer);
        _logTimer = null;
        ...
        console.warn('[Drive] Pre-sync push failed (non-fatal):', e.message);
      }
