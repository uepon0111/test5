...
const UI = (() => {
  ...
  let _artists   = [];   // all artists from Storage (v2)
  ...
  let _overviewPeriodMode   = 'month';  // 'week' | 'month'
  let _overviewPeriodOffset = 0;        // 0=current, -1=prev, etc.
  ...
  let _editCurrentArtistIds = []; // artist id list being edited (v2)
  ...
  function applySort() {
    ...
    // NOTE: do NOT set sortSel.value here — it overwrites user input during change events
    const sortSel = document.getElementById('pl-sort-select');
    if (sortSel && sortSel !== document.activeElement) sortSel.value = _sortKey;
  }
  ...
  async function renderLogOverview() {
    // Use in-memory _tracks (has _thumbUrl attached) instead of re-fetching from Storage
    ...
    // Artist stats: resolve both artistIds and legacy artist string
    const artistTimeSec  = {};
    const artistPlaysCnt = {};
    logs.forEach(l => {
      const t = tracks.find(t2 => t2.id === l.trackId);
      if (!t) return;
      const keys = (t.artistIds && t.artistIds.length > 0)
        ? t.artistIds
        : [t.artist || '不明なアーティスト'];
      keys.forEach(k => {
        artistTimeSec[k]  = (artistTimeSec[k]  || 0) + (l.duration || 0);
        artistPlaysCnt[k] = (artistPlaysCnt[k] || 0) + 1;
      });
    });

    const topTrackId = Object.entries(playCounts).sort((a,b) => b[1]-a[1])[0]?.[0];
    const topTrack   = tracks.find(t => t.id === topTrackId);
    const topArtistKey = Object.entries(artistTimeSec).sort((a,b) => b[1]-a[1])[0]?.[0] || '—';

    // Resolve top artist display name
    let topArtistName = topArtistKey;
    if (topArtistKey !== '—' && _artists.length > 0) {
      const a = _artists.find(a => a.id === topArtistKey);
      if (a) topArtistName = a.name;
    }
    ...
    if (grid) {
      ...
      const artistCount = _artists.length > 0
        ? _artists.length
        : new Set(tracks.map(t => t.artist).filter(Boolean)).size;
      grid.innerHTML = `
        <div class="stat-card">
        ...
        <div class="stat-card">
          ...
          <div class="stat-sub">${artistCount} アーティスト</div>
        </div>
        ...
        <div class="stat-card">
          ...
          <div class="stat-value">${logs.length.toLocaleString()}<span class="stat-unit">回</span></div>
          <div class="stat-sub">記録のある日数: ${new Set(logs.map(l => new Date(l.playedAt).toDateString())).size}日</div>
        </div>`;
    }

    // ── PERIOD HIGHLIGHTS ────────────────────────────────────────
    await _renderPeriodHighlights(tracks, logs, playCounts, trackTime);
  ...
  /**
   * 期間別ハイライト — モックデザイン準拠
   * 横スクロールカードUI、アーティストアイコン対応、期間ナビ付き
   */
  async function _renderPeriodHighlights(tracks, logs, allPlayCounts, allTrackTime) {
    ...
    const navHtml = `
      <div class="period-nav-bar">
        <div class="period-nav-mode">
          <button class="inner-tab ${_overviewPeriodMode==='week' ? 'active':''}"
            onclick="UI.switchOverviewPeriodMode('week')">週</button>
          <button class="inner-tab ${_overviewPeriodMode==='month' ? 'active':''}"
            onclick="UI.switchOverviewPeriodMode('month')">月</button>
        </div>
        <button class="btn-icon" onclick="UI.overviewPeriodPrev()" title="前の期間">
          <i class="fa-solid fa-chevron-left"></i>
        </button>
        <span id="overview-period-label" style="font-size:13px;color:var(--text-secondary);font-weight:500;min-width:90px;text-align:center"></span>
        <button class="btn-icon" id="overview-period-next" onclick="UI.overviewPeriodNext()" title="次の期間">
    ...
    if (_overviewPeriodMode === 'week') {
      const refDay = new Date(now);
      refDay.setDate(refDay.getDate() + _overviewPeriodOffset * 7);
      const dow = refDay.getDay();
      const wStart = new Date(refDay); wStart.setDate(refDay.getDate() - dow); wStart.setHours(0,0,0,0);
      const wEnd   = new Date(wStart); wEnd.setDate(wStart.getDate() + 6);     wEnd.setHours(23,59,59,999);
      periodStart = wStart.getTime();
      periodEnd   = wEnd.getTime();
      const fmt = d => `${d.getMonth()+1}/${d.getDate()}`;
      periodLabel = `${fmt(wStart)} 〜 ${fmt(wEnd)}`;
    } else {
      const ref = new Date(now.getFullYear(), now.getMonth() + _overviewPeriodOffset, 1);
      periodStart = new Date(ref.getFullYear(), ref.getMonth(), 1).getTime();
      periodEnd   = new Date(ref.getFullYear(), ref.getMonth()+1, 0, 23, 59, 59, 999).getTime();
      periodLabel = `${ref.getFullYear()}年${ref.getMonth()+1}月`;
    }

    // 「次の期間」ボタンは現在より未来は無効
    const isAtPresent = _overviewPeriodOffset >= 0;

    // ── 集計 ────────────────────────────────────────────────────
    const periodLogs = logs.filter(l => l.playedAt >= periodStart && l.playedAt <= periodEnd);
    const totalPeriodSec = periodLogs.reduce((s,l) => s+(l.duration||0), 0);

    // Track time
    const pTrackTime = {};
    // Artist time (key = artistId or artist string)
    const pArtistTimeSec  = {};
    const pArtistPlaysCnt = {};

    periodLogs.forEach(l => {
      pTrackTime[l.trackId] = (pTrackTime[l.trackId] || 0) + (l.duration || 0);
      const t = tracks.find(t2 => t2.id === l.trackId);
      if (!t) return;
      const keys = (t.artistIds && t.artistIds.length > 0) ? t.artistIds : [t.artist || '不明'];
      keys.forEach(k => {
        pArtistTimeSec[k]  = (pArtistTimeSec[k]  || 0) + (l.duration || 0);
        pArtistPlaysCnt[k] = (pArtistPlaysCnt[k] || 0) + 1;
      });
    });

    // ── アーティストカード HTML ─────────────────────────────────
    const topArtistKeys = Object.entries(pArtistTimeSec)
      .sort((a,b) => b[1]-a[1]).slice(0,5).map(([k])=>k);

    const artistCards = await Promise.all(topArtistKeys.map(async (key, idx) => {
      // Resolve artist
      let name = key, iconUrl = null;
      const managedArtist = _artists.find(a => a.id === key);
      if (managedArtist) {
        name = managedArtist.name;
        if (managedArtist.iconKey) {
          iconUrl = await Storage.getBlobUrl(managedArtist.iconKey).catch(() => null);
        }
      }
      // Fallback: representative track thumb
      if (!iconUrl) {
        const repTrackId = Object.entries(pTrackTime)
          .filter(([id]) => {
            const t = tracks.find(t2 => t2.id === id);
            if (!t) return false;
            return (t.artistIds && t.artistIds.includes(key)) || t.artist === key;
          })
          .sort((a,b) => b[1]-a[1])[0]?.[0];
        const repTrack = repTrackId ? tracks.find(t2 => t2.id === repTrackId) : null;
        iconUrl = repTrack?._thumbUrl || null;
      }
      const thumbHtml = iconUrl
        ? `<img src="${iconUrl}" style="width:100%;height:100%;object-fit:cover">`
        : `<i class="fa-solid fa-music" style="font-size:24px;opacity:0.3"></i>`;
      const isTop = idx === 0;
      return `<div class="period-mini-card${isTop?' top-card':''}">
        <div class="period-mini-art">
          ${thumbHtml}
          ${isTop ? '<div class="top-flag">TOP</div>' : ''}
          <div class="val-overlay">${_fmtSec(pArtistTimeSec[key]||0)}</div>
        </div>
        <div class="period-mini-info">
          <div class="period-mini-title">${_esc(name)}</div>
          <div class="period-mini-sub">${pArtistPlaysCnt[key]||0} 再生</div>
        </div>
      </div>`;
    }));
    ...
    const trackCards = topTrackIds.map((id, idx) => {
      const track = tracks.find(t2 => t2.id === id);
      if (!track) return '';
      const thumbHtml = track._thumbUrl
        ? `<img src="${track._thumbUrl}" style="width:100%;height:100%;object-fit:cover">`
        : `<i class="fa-solid fa-music" style="font-size:24px;opacity:0.3"></i>`;
      // Artist display name (use managed artist name if available)
      const artistName = _getTrackArtistDisplaySync(track);
      const isTop = idx === 0;
      return `<div class="period-mini-card${isTop?' top-card':''}">
        <div class="period-mini-art">
          ${thumbHtml}
          ${isTop ? '<div class="top-flag">TOP</div>' : ''}
          <div class="val-overlay">${_fmtSec(pTrackTime[id]||0)}</div>
        </div>
        <div class="period-mini-info">
          <div class="period-mini-title">${_esc(track.title)}</div>
          <div class="period-mini-sub">${_esc(artistName)}</div>
        </div>
      </div>`;
    });

    const noData = periodLogs.length === 0;
    const bodyHtml = noData
      ? '<p style="color:var(--text-muted);font-size:13px;padding:12px 0">この期間の再生履歴がありません</p>'
      : `<div class="period-cols">
          <div>
            <div class="period-col-head"><i class="fa-solid fa-user-music"></i>アーティスト</div>
            <div class="period-scroll">${artistCards.join('')}</div>
          </div>
          <div>
            <div class="period-col-head"><i class="fa-solid fa-music"></i>楽曲</div>
            <div class="period-scroll">${trackCards.join('')}</div>
          </div>
        </div>`;
    ...
    container.innerHTML = `
      ...
      <div class="section-card period-highlight-card">
        <div class="section-card-title">
          ...
          <span id="overview-period-label-inner">${_esc(periodLabel)}</span>
          <span class="period-total-time">${_fmtSec(totalPeriodSec)}</span>
          <button class="btn-icon period-nav-btn-inline" id="overview-period-next-inline"
            onclick="UI.overviewPeriodNext()" ${isAtPresent?'disabled':''} style="margin-left:auto">
            <i class="fa-solid fa-chevron-right"></i>
          </button>
        </div>
        ${bodyHtml}
    ...
    // Disable next button if at present
    const nextBtns = container.querySelectorAll('#overview-period-next, #overview-period-next-inline');
    nextBtns.forEach(b => { b.disabled = isAtPresent; });

    // Update period label
    const lbl = container.querySelector('#overview-period-label');
    if (lbl) lbl.textContent = periodLabel;
  }

  /** Get artist display name synchronously from in-memory _artists */
  function _getTrackArtistDisplaySync(track) {
    if (track.artistIds && track.artistIds.length > 0 && _artists.length > 0) {
      const names = track.artistIds
        .map(id => _artists.find(a => a.id === id)?.name)
        .filter(Boolean);
      if (names.length > 0) return names.join(', ');
    }
    return track.artist || '不明なアーティスト';
  }

  // ── 期間ナビ API ────────────────────────────────────────────
  function switchOverviewPeriodMode(mode) {
    _overviewPeriodMode   = mode;
    _overviewPeriodOffset = 0;
    renderLogOverview();
  }
  function overviewPeriodPrev() {
    _overviewPeriodOffset--;
    renderLogOverview();
  }
  function overviewPeriodNext() {
    if (_overviewPeriodOffset >= 0) return;
    _overviewPeriodOffset++;
    renderLogOverview();
  }
  ...
  function _renderBarChart(tracks, logs) {
    ...
    chart.innerHTML = data.map((v, i) => {
      ...
      return `<div class="bar-col" title="${labels[i]}: ${tip}">
        ${showLabel ? `<div class="bar-col-time">${tip}</div>` : '<div class="bar-col-time" style="opacity:0">-</div>'}
    ...
    rank.innerHTML = sorted.map(([id, dur], i) => {
      ...
      return `<div class="ranking-item">
        ...
        <div class="ranking-track-meta">
          ...
          <span class="ranking-artist">${_esc(track ? _getTrackArtistDisplaySync(track) : '')}</span>
        </div>
  ...
  async function renderArtistManager() {
    ...
    if (_artists.length === 0) {
      list.innerHTML = '<div class="empty-state"><i class="fa-solid fa-user-music"></i><p>アーティストがありません</p></div>';
      return;
    }
    ...
    const rows = await Promise.all(_artists.map(async (artist, i) => {
      let iconHtml = `<div class="artist-icon-placeholder"><i class="fa-solid fa-user-music"></i></div>`;
      if (artist.iconKey) {
        const url = await Storage.getBlobUrl(artist.iconKey).catch(() => null);
        if (url) iconHtml = `<img src="${url}" class="artist-icon-img" alt="${_esc(artist.name)}">`;
      }
      return `<div class="artist-manager-item" data-id="${artist.id}">
        <div class="artist-icon-wrap">${iconHtml}</div>
        <span class="artist-item-name">${_esc(artist.name)}</span>
        <span class="artist-item-count">${counts[artist.id] || 0}曲</span>
        <div class="tag-order-btns">
          <button class="btn-icon" ${i===0?'disabled':''} onclick="UI._moveArtist('${artist.id}',-1)" title="上へ">
            <i class="fa-solid fa-arrow-up"></i>
          </button>
          <button class="btn-icon" ${i===_artists.length-1?'disabled':''} onclick="UI._moveArtist('${artist.id}',1)" title="下へ">
            <i class="fa-solid fa-arrow-down"></i>
          </button>
        </div>
        <button class="btn-icon" onclick="UI.openEditArtistModal('${artist.id}')" title="編集">
          <i class="fa-solid fa-pen"></i>
        </button>
        <button class="btn-icon" onclick="UI._confirmDeleteArtist('${artist.id}')" title="削除" style="color:var(--danger)">
          <i class="fa-solid fa-trash"></i>
        </button>
      </div>`;
    }));
  ...
  function _confirmDeleteArtist(id) {
    const artist = _artists.find(a => a.id === id);
    _confirmCallback = () => App.deleteArtist(id);
    _showConfirm('アーティストを削除', `「${artist?.name || ''}」を削除します。曲からも解除されます。`);
  }

  let _artistEditId      = null;
  ...
  function openNewArtistModal() {
    ...
    document.getElementById('artist-modal-title').textContent    = 'アーティストを作成';
    document.getElementById('artist-modal-btn-txt').textContent  = '作成';
    document.getElementById('artist-name-input').value           = '';
    document.getElementById('artist-edit-id').value              = '';
    const iconBtn = document.getElementById('artist-icon-btn');
    if (iconBtn) iconBtn.innerHTML = '<i class="fa-solid fa-user-music"></i><input type="file" id="artist-icon-input" accept="image/*" style="display:none">';
  ...
  async function openEditArtistModal(id) {
    ...
    document.getElementById('artist-modal-btn-txt').textContent  = '保存';
    document.getElementById('artist-name-input').value           = artist.name;
    document.getElementById('artist-edit-id').value              = id;
    const iconBtn = document.getElementById('artist-icon-btn');
    if (iconBtn) {
      if (artist.iconKey) {
        const url = await Storage.getBlobUrl(artist.iconKey).catch(() => null);
        iconBtn.innerHTML = url
          ...
          : '<i class="fa-solid fa-user-music"></i><input type="file" id="artist-icon-input" accept="image/*" style="display:none">';
      } else {
        iconBtn.innerHTML = '<i class="fa-solid fa-user-music"></i><input type="file" id="artist-icon-input" accept="image/*" style="display:none">';
      }
      document.getElementById('artist-icon-input')?.addEventListener('change', _onArtistIconSelected);
    }
    openModal('artist-modal');
  ...
  async function _onArtistIconSelected(e) {
    const file = e.target.files[0];
    if (!file) return;
    _artistIconData = await file.arrayBuffer();
    _artistIconMime = file.type || 'image/jpeg';
    const url = URL.createObjectURL(file);
    const btn = document.getElementById('artist-icon-btn');
    const inp = document.getElementById('artist-icon-input');
    btn.innerHTML = `<img src="${url}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
    btn.appendChild(inp);
  }

  function getArtistFormData() {
    return {
      id:       document.getElementById('artist-edit-id').value  || null,
  ...
  async function openEditTrackModal(trackId, e) {
    ...
    _editCurrentArtistIds = [...(track.artistIds || [])];
    ...
    // Datalists
    const legacyArtists = [...new Set(_tracks.map(t => t.artist).filter(Boolean))];
    document.getElementById('artist-datalist').innerHTML =
      legacyArtists.map(a => `<option value="${_esc(a)}">`).join('');
    document.getElementById('tag-datalist').innerHTML =
      _tags.map(t => `<option value="${_esc(t.name)}">`).join('');

    // Artist ID datalist from managed artists
    const aidDl = document.getElementById('artist-id-datalist');
    if (aidDl) aidDl.innerHTML = _artists.map(a => `<option value="${_esc(a.name)}">`).join('');
  ...
  function _renderEditArtistPills() {
    ...
    if (!container) return;
    container.innerHTML = _editCurrentArtistIds.map(id => {
      const artist = _artists.find(a => a.id === id);
      if (!artist) return '';
      return `<button class="tag-pill" style="background:var(--accent-light);color:var(--accent)"
        onclick="UI._removeArtistFromEdit('${id}')">
        <i class="fa-solid fa-user-music" style="font-size:10px;margin-right:3px"></i>${_esc(artist.name)}
        <span class="tag-pill-remove"><i class="fa-solid fa-xmark"></i></span>
      </button>`;
    }).join('');
  }

  function _removeArtistFromEdit(id) {
    _editCurrentArtistIds = _editCurrentArtistIds.filter(a => a !== id);
    _renderEditArtistPills();
  }

  function _addArtistToEdit() {
    const input = document.getElementById('edit-artist-id-input');
    if (!input) return;
    const name = input.value.trim();
    const artist = _artists.find(a => a.name.toLowerCase() === name.toLowerCase());
    if (artist && !_editCurrentArtistIds.includes(artist.id)) {
      _editCurrentArtistIds.push(artist.id);
      _renderEditArtistPills();
    }
    input.value = '';
  }
  ...
  async function getEditFormData() {
    return {
      ...
      artistIds:   [..._editCurrentArtistIds],
  ...
  return {
    ...
    calNext,
    _calDayClick,
    // Period highlights navigation
    switchOverviewPeriodMode,
    overviewPeriodPrev,
    overviewPeriodNext,
