...
const UI = (() => {
  ...
  async function renderLogOverview() {
    ...
    if (artistBoardSec && artistBoardEl) {
      ...
      if (sortedArtists.length) {
        ...
        // アーティストアイコン取得マップ
        const allArtistRecs = await Storage.getArtists();
        const artistIconMap  = {};
        await Promise.all(allArtistRecs.map(async a => {
        ...
        const rowsHtml = await Promise.all(sortedArtists.map(async ([artist, sec], i) => {
          ...
          let avatarHtml;
          const iconUrl = artistIconMap[artist];
          if (iconUrl) {
  ...
  async function _renderPeriodHighlights(tracks, logs) {
    ...
    // アーティストレコードマップ（アイコン取得用）
    const allArtists  = await Storage.getArtists();
    const artistMap   = {};
    await Promise.all(allArtists.map(async a => {
      const url = a.iconKey ? await Storage.getBlobUrl(a.iconKey).catch(() => null) : null;
      artistMap[a.name] = { ...a, _iconUrl: url };
    }));
    ...
    const artistCardsHtml = topArtists.map(([artist, sec], idx) => {
      ...
      // アーティストアイコン: artistMap から取得、なければ代表トラックのサムネイル
      const artRec = artistMap[artist];
      let artImg;
      if (artRec?._iconUrl) {
        artImg = `<img src="${artRec._iconUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
      } else {
