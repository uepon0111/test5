...
const UI = (() => {
  ...
  let _artists   = [];   // all artists from Storage
  ...
  function _renderTrackItem(track, idx) {
    ...
    div.innerHTML = `
      ...
      <div class="track-meta">
        ...
        <div class="track-artist-row">${_esc(_getTrackArtistLabel(track))}</div>
  ...
  async function loadData() {
    [_tracks, _tags, _artists, _playlists] = await Promise.all([
      Storage.getTracks(),
      Storage.getTags(),
      Storage.getArtists(),
  ...
  function _splitArtistNames(value) {
    if (!value) return [];
    return String(value)
      .split(/\s*(?:、|,|\/|&|feat\.?|ft\.?|\+)\s*/i)
      .map(s => s.trim())
  ...
  function _getArtistById(id) {
    return _artists.find(a => a.id === id) || null;
  }

  function _getTrackArtistIds(track) {
    return Array.isArray(track?.artistIds) ? track.artistIds.filter(Boolean) : [];
  }

  function _getTrackArtistNames(track) {
    const ids = _getTrackArtistIds(track);
    if (ids.length) {
      const names = ids.map(id => _getArtistById(id)?.name).filter(Boolean);
      if (names.length) return names;
    }
    return _splitArtistNames(track?.artist || '');
  }

  function _getTrackArtistLabel(track) {
    const names = _getTrackArtistNames(track);
    return names.length ? names.join('、') : (track?.artist || '—');
  }

  function _getTrackArtistIcons(track) {
    const ids = _getTrackArtistIds(track);
    if (!ids.length) return [];
    return ids.map(id => _getArtistById(id)).filter(Boolean);
  }
  ...
  function applySort() {
    ...
    if (_searchQuery.trim()) {
      ...
      list = list.filter(t =>
        ...
        _getTrackArtistLabel(t).toLowerCase().includes(q) ||
  ...
  async function renderLogOverview() {
    ...
    logs.forEach(l => {
      ...
      if (t) {
        const names = _getTrackArtistNames(t);
        if (!names.length) {
          artistTime[t.artist || '']  = (artistTime[t.artist || '']  || 0) + (l.duration || 0);
          artistPlays[t.artist || ''] = (artistPlays[t.artist || ''] || 0) + 1;
        } else {
          names.forEach(name => {
            artistTime[name]  = (artistTime[name]  || 0) + (l.duration || 0);
            artistPlays[name] = (artistPlays[name] || 0) + 1;
          });
        }
      }
    });
    ...
    if (grid) {
      ...
      grid.innerHTML = `
        ...
        <div class="stat-card">
          ...
          <div class="stat-sub">${new Set(tracks.flatMap(t => _getTrackArtistNames(t))).size} アーティスト</div>
        </div>
    ...
    // 期間別ハイライト
    await _renderPeriodHighlights(tracks, logs, playCounts, trackTime, artistTime, artistPlays);
  ...
  /**
   * 期間別ハイライト: 直近3ヶ月を1ヶ月刻みで集計し、
   * よく聴いたアーティスト上位4件・よく聴いた曲上位4件・総再生時間を表示する
   */
  async function _renderPeriodHighlights(tracks, logs, allPlayCounts, allTrackTime, allArtistTime, allArtistPlays) {
    ...
    for (const period of periods) {
      ...
      periodLogs.forEach(l => {
        ...
        if (t) {
          ...
          if (!names.length) {
            ...
            pArtistPlay[t.artist || ''] = (pArtistPlay[t.artist || ''] || 0) + 1;
          } else {
            names.forEach(name => {
              pArtistTime[name] = (pArtistTime[name] || 0) + (l.duration || 0);
              pArtistPlay[name] = (pArtistPlay[name] || 0) + 1;
            });
          }
        }
      });
      ...
      // アーティストの代表サムネイル: そのアーティストの最再生曲のサムネ
      const artistThumbHtml = async (artist) => {
        const artistTrackIds = Object.entries(pTrackTime)
          ...
          .sort((a, b) => b[1] - a[1]);
        const repTrack = artistTrackIds.length
          ? tracks.find(t2 => t2.id === artistTrackIds[0][0]) : null;
      ...
      const artistRows = await Promise.all(topArtists.map(async (artist, idx) => {
        const thumbInner = await artistThumbHtml(artist);
        const isFirst = idx === 0;
        return `<div class="period-artist-row${isFirst ? ' period-artist-row--top' : ''}">
      ...
      const trackRows = await Promise.all(topTrackIds.map(async (id, idx) => {
        ...
        if (!track) return '';
        const url = track._thumbUrl || null;
        const thumbInner = url
          ? `<img src="${url}" style="width:100%;height:100%;object-fit:cover;border-radius:4px">`
          : `<i class="fa-solid fa-music" style="font-size:14px;opacity:0.5"></i>`;
        const isFirst = idx === 0;
        return `<div class="period-track-row${isFirst ? ' period-track-row--top' : ''}">
          <div class="period-item-thumb">${thumbInner}</div>
          <div class="period-track-meta">
            <div class="period-track-title">${_esc(track.title)}</div>
            <div class="period-track-artist">${_esc(_getTrackArtistLabel(track))}</div>
          </div>
    ...
    container.innerHTML = sectionsHtml.join('') ||
      '<p style="color:var(--text-muted);font-size:13px;padding:4px 0 12px">再生履歴がありません</p>';
  }
    ...
    list.innerHTML = items.map(({ track, years }) => {
      ...
      return `<div class="anniv-item">
        ...
        <div class="anniv-info">
          ...
          <div class="anniv-sub">${_esc(_getTrackArtistLabel(track))} · 投稿日: ${track.releaseDate}</div>
        </div>
        ${tagHtml}
      </div>`;
    }).join('');
  ...
  function _renderBarChart(tracks, logs) {
    ...
    chart.innerHTML = data.map((v, i) => {
      ...
      return `<div class="bar-col" title="${labels[i]} · ${tip}">
        <div class="bar-col-value">${tip}</div>
    ...
    rank.innerHTML = sorted.map(([id, dur], i) => {
      ...
      return `<div class="ranking-item">
        ...
        <div class="ranking-track-meta">
          ...
          <span class="ranking-artist">${_esc(track ? _getTrackArtistLabel(track) : '')}</span>
        </div>
  ...
  function switchGraphCat(btn) {
    const tabs = document.getElementById('graph-category-tabs');
    if (tabs) {
      tabs.querySelectorAll('.inner-tab').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
  ...
  function _calDayClick(day) {
    ...
    elist.innerHTML = events.map(({ track, type }) => {
      ...
      return `<div class="anniv-item">
        ...
        <div class="anniv-info">
          <div class="anniv-title">${_esc(track.title)}</div>
          <div class="anniv-sub">${_esc(_getTrackArtistLabel(track))} · ${track.releaseDate}</div>
        </div>
      </div>`;
    }).join('');
  }
  ...
  async function renderEditGrid() {
    ...
    switch (_editSortKey) {
      ...
      case 'artist':
        list.sort((a,b) => dir*(_getTrackArtistLabel(a)).localeCompare(_getTrackArtistLabel(b),'ja')); break;
      case 'date':
    ...
    grid.innerHTML = list.map(t => {
      ...
      const artistLabel = _getTrackArtistLabel(t);
      ...
      return `<div class="edit-card" onclick="UI.openEditTrackModal('${t.id}',event)">
        ...
        <div class="edit-card-info">
          ...
          <div class="edit-card-artist" title="${_esc(artistLabel)}">${_esc(artistLabel)}</div>
          ${dateHtml}
  ...
  async function renderArtistManager() {
    ...
    list.innerHTML = _artists.map((artist, i) => `
      <div class="tag-manager-item" data-id="${artist.id}">
        <div class="artist-icon-swatch" style="background:${artist.color};color:${artist.textColor};border:2px solid ${artist.textColor}40">
          <i class="${artist.icon || 'fa-solid fa-user-music'}"></i>
        </div>
        <span class="tag-item-name">${_esc(artist.name)}</span>
        <span class="tag-item-count">${counts[artist.name] || 0}曲</span>
        <div class="tag-order-btns">
  ...
  async function _moveArtist(id, dir) {
    const idx = _artists.findIndex(t => t.id === id);
    if (idx < 0) return;
    const newIdx = idx + dir;
    if (newIdx < 0 || newIdx >= _artists.length) return;
    [_artists[idx], _artists[newIdx]] = [_artists[newIdx], _artists[idx]];
    await Storage.reorderArtists(_artists.map(t => t.id));
    renderArtistManager();
  }

  function _confirmDeleteArtist(id) {
    const artist = _artists.find(t => t.id === id);
    _confirmCallback = () => App.deleteArtist(id);
    _showConfirm('アーティストを削除', `アーティスト「${artist?.name || ''}」を削除します。曲からも削除されます。`);
  }
  ...
  function openNewArtistModal() {
    ...
    document.getElementById('artist-icon-input').value = 'fa-solid fa-user-music';
    document.getElementById('artist-edit-id').value = '';
    document.getElementById('artist-preview-icon').className = 'fa-solid fa-user-music';
    document.getElementById('artist-preview-name').textContent = 'アーティスト名';
    document.getElementById('artist-preview-dot').style.background = _tagColor;
  ...
  async function openEditArtistModal(id) {
    ...
    if (!artist) return;
    _tagEditId    = id;
    _tagColor     = artist.color || '#DBEAFE';
    _tagTextColor = artist.textColor || '#1D4ED8';
    ...
    document.getElementById('artist-icon-input').value = artist.icon || 'fa-solid fa-user-music';
    document.getElementById('artist-edit-id').value = id;
    document.getElementById('artist-preview-icon').className = artist.icon || 'fa-solid fa-user-music';
    document.getElementById('artist-preview-name').textContent = artist.name || 'アーティスト名';
    document.getElementById('artist-preview-dot').style.background = _tagColor;
    openModal('artist-modal');
  }

  function selectArtistIcon(iconClass) {
    document.getElementById('artist-icon-input').value = iconClass;
    document.getElementById('artist-preview-icon').className = iconClass;
  }

  function selectArtistCustomColor(hexColor) {
  ...
  function getArtistFormData() {
    return {
      ...
      icon: document.getElementById('artist-icon-input').value.trim() || 'fa-solid fa-user-music',
      color: _tagColor,
  ...
  async function openEditTrackModal(trackId, e) {
    ...
    document.getElementById('edit-artist').value   = _getTrackArtistLabel(track)  || '';
    ...
    // Datalists
    const artists = _artists.length
      ? _artists.map(a => a.name)
      : [...new Set(_tracks.flatMap(t => _splitArtistNames(t.artist)).filter(Boolean))];
  ...
  async function getEditFormData() {
    const artistNames = [...new Set(_splitArtistNames(document.getElementById('edit-artist').value.trim()))];
    return {
      id:          _editTrackId,
