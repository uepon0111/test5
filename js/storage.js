...
const Storage = (() => {
  ...
  const DB_VERSION = 2;
  const STORES     = ['tracks','blobs','playlists','tags','artists','logs','meta'];
  let _db = null;
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = e => {
        ...
        if (!db.objectStoreNames.contains('artists')) {
          const ars = db.createObjectStore('artists', { keyPath:'id' });
          ars.createIndex('by_order','order',{ unique:false });
        }
  ...
  async function _resolveArtistNames(ids) {
    if (!ids || !ids.length) return [];
    const artists = await getArtists().catch(() => []);
    const map = new Map(artists.map(a => [a.id, a]));
    return ids.map(id => map.get(id)?.name).filter(Boolean);
  }
  async function getArtists() {
    const artists = await getAll('artists','by_order').catch(() => []);
    const sorted = artists.sort((a,b) => (a.order||0)-(b.order||0));
    if (sorted.length) return sorted;

    // Seed from existing tracks on first run.
    const tracks = await getTracks();
    const names = [];
    const seen = new Set();
    for (const t of tracks) {
      const parts = _splitArtistNames(t.artist);
    ...
    for (const name of names) {
      const artist = { id: uid(), name, icon:'fa-solid fa-user-music', color:'#DBEAFE', textColor:'#1D4ED8', order: order++ };
      await put('artists', artist);
    }
    return getAll('artists','by_order').then(list => list.sort((a,b) => (a.order||0)-(b.order||0)));
  }
  const getArtist = id => getOne('artists', id);
  async function createArtist(name, icon='fa-solid fa-user-music', color='#DBEAFE', textColor='#1D4ED8') {
    const existing = await getArtists();
    const artist = {
      id: uid(),
      name: name.trim() || '不明なアーティスト',
      icon,
      color,
      textColor,
      order: existing.length,
    };
    await put('artists', artist); return artist;
  }
  async function updateArtist(id, changes) {
    const artist = await getArtist(id);
    if (!artist) throw new Error('Artist not found: ' + id);
    const updated = { ...artist, ...changes };
    await put('artists', updated); return updated;
  }
  async function deleteArtist(id) {
    ...
    const tracks = await getTracks();
    for (const t of tracks) {
      const ids = (t.artistIds || []).filter(aid => aid !== id);
      const currentNames = ids.length ? await _resolveArtistNames(ids) : _splitArtistNames(t.artist);
      const fallbackNames = _splitArtistNames(t.artist).filter(n => n !== name);
      const names = ids.length ? currentNames : fallbackNames;
      const shouldUpdate = (t.artistIds || []).includes(id) || _splitArtistNames(t.artist).includes(name);
      if (shouldUpdate) {
  ...
  function reorderArtists(orderedIds) {
    return new Promise((resolve, reject) => {
      const t     = _db.transaction('artists', 'readwrite');
      const store = t.objectStore('artists');
      t.oncomplete = () => resolve();
      t.onerror    = e => reject(e.target.error);
      t.onabort    = () => reject(new Error('reorderArtists aborted'));
      orderedIds.forEach((id, idx) => {
        const r = store.get(id);
        r.onsuccess = e => {
          const artist = e.target.result;
          if (artist) store.put({ ...artist, order: idx });
        };
        r.onerror = () => t.abort();
      });
    });
  ...
  async function exportSnapshot() {
    const [tracks, playlists, tags, artists, logs] = await Promise.all([
      getTracks(), getPlaylists(), getTags(), getArtists(), getLogs()
    ]);
    return {
      version:    1,
      exportedAt: Date.now(),
      tracks:     tracks.map(({ blobKey, thumbKey, ...rest }) => rest),
      playlists, tags, artists, logs,
    };
  }
  async function importSnapshot(snapshot) {
    ...
    for (const t of snapshot.tracks||[]) {
      ...
      if (!ex) {
        await put('tracks', { ...t, blobKey: t.blobKey ?? null, thumbKey: t.thumbKey ?? null });
      } else {
        await put('tracks', {
          ...ex,
          ...t,
          blobKey: ex.blobKey ?? t.blobKey ?? null,
          thumbKey: ex.thumbKey ?? t.thumbKey ?? null,
        });
      }
    }
    ...
    for (const pl of snapshot.playlists||[]) {
      ...
      else await put('playlists', { ...ex, ...pl, trackIds:[...new Set([...(ex.trackIds||[]), ...(pl.trackIds||[])])] });
    }

    for (const tag of snapshot.tags||[]) {
      const ex = await getTag(tag.id);
      if (!ex) await put('tags', tag);
      else await put('tags', { ...ex, ...tag });
    }

    for (const artist of snapshot.artists||[]) {
      const ex = await getArtist(artist.id);
      if (!ex) await put('artists', artist);
      else await put('artists', { ...ex, ...artist });
    }
