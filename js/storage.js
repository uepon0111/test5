 ...
 * v2: Added `artists` store; tracks support `artistIds[]` for multiple artists.
 */
...
const Storage = (() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = e => {
        ...
        if (!db.objectStoreNames.contains('artists')) {
          ...
          as.createIndex('by_order','order',{ unique:false });
  ...
  /* ═══════════════════════════════════════════════════════════
     ARTISTS  (v2)
     { id, name, iconKey, order, createdAt }
  ══════════════════════════════════════════════════════════ */
  async function getArtists() {
    const artists = await getAll('artists', 'by_order');
    return artists.sort((a,b) => (a.order||0)-(b.order||0));
  }
  const getArtist = id => getOne('artists', id);

  async function createArtist(name, iconKey=null) {
  ...
  async function deleteArtist(id) {
    const tracks = await getTracks();
    for (const t of tracks)
      if ((t.artistIds||[]).includes(id))
        await updateTrack(t.id, { artistIds: t.artistIds.filter(a => a !== id) });
    const artist = await getArtist(id);
    if (artist?.iconKey) await deleteBlob(artist.iconKey);
  ...
  /**
   * Resolve artist info for a track.
   * Returns array of { id, name, iconKey }.
   * Falls back to legacy `artist` string.
   */
  async function resolveArtistNames(track) {
    ...
    if (track.artist && track.artist !== '不明なアーティスト') {
      return [{ id: null, name: track.artist, iconKey: null }];
    }
    return [];
  }

  async function getArtistDisplayName(track) {
    const artists = await resolveArtistNames(track);
    if (artists.length === 0) return track.artist || '不明なアーティスト';
  ...
  async function importSnapshot(snapshot) {
    ...
    // Tracks — add new; merge artistIds if missing locally
    for (const t of snapshot.tracks||[]) {
      ...
      } else if (!ex.artistIds?.length && t.artistIds?.length) {
        await put('tracks', { ...ex, artistIds: t.artistIds });
      }
    }
    ...
    // Logs — append-only, never overwrite or delete
    ...
    for (const artist of snapshot.artists||[]) {
      ...
      if (!ex) {
        await put('artists', { ...artist, iconKey: null });
      } else if (ex.name !== artist.name) {
        await put('artists', { ...ex, name: artist.name });
      }
    }
  }
  ...
  async function resetAll() {
    for (const s of STORES) {
      try { await clearStore(s); } catch { /* store may not exist in older DB */ }
    }
  }
  ...
  function _id3APIC(data) {
    ...
    return { data: imgData.buffer, mime: _imgMime(imgData) };
  }
  ...
  return {
    ...
    resolveArtistNames, getArtistDisplayName,
    getLogs, addLog,
